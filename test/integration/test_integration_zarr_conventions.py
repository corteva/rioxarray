...
def test_read_crs_from_proj_code():
...
def test_read_crs_from_proj_wkt2():
...
def test_read_transform_from_spatial_transform():
...
def test_spatial_dimensions_takes_precedence():
...
def test_zarr_conventions_priority_over_cf():
    ...
    # TODO: Add warning when both conventions are present
    crs = da.rio.crs
    assert crs.to_epsg() == 3857


def test_cf_conventions_as_fallback():
...
def test_group_level_proj_inheritance_dataset():
...
def test_write_zarr_crs_wkt2():
    """Test writing CRS as proj:wkt2 (default format)."""
...
def test_write_zarr_spatial_metadata():
...
def test_write_zarr_spatial_metadata_with_bbox():
...
def test_write_zarr_conventions_all():
    ...
    # Check CRS attributes (WKT2 format only now)
    assert "proj:wkt2" in da.attrs
...
def test_roundtrip_proj_wkt2():
    """Test write then read of proj:wkt2 (default format)."""
    from rioxarray._convention import zarr
    ...
    # Verify format
    assert "proj:wkt2" in original_da.attrs
...
def test_roundtrip_spatial_transform():
...
def test_roundtrip_complete_conventions():
...
def test_both_conventions_present():
    ...
    assert "proj:wkt2" in da.attrs  # Zarr convention

    # Zarr should take priority when reading
    assert da.rio.crs.to_epsg() == 4326


def test_zarr_overrides_cf_when_both_present():
...
def test_invalid_spatial_transform_format():
...
def test_write_crs_without_setting():
...
def test_write_spatial_metadata_without_dimensions():
...
def test_crs_from_projjson_dict():
